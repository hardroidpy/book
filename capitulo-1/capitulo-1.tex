...
El reconocimiento de actividades humanas (\abbr{HAR}) es un tópico
...
automático (\abbr{ML}), donde se utiliza un modelo de inferencia
...
\label{sec12:justificaciuxf3n}Construir un sistema \abbr{HAR} requiere
...
sensores como: localización (\abbr{GPS}), brújula, aceleración, audio
...
movimiento en varias dimensiones, y siendo combinado con otros puede
detectar la orientación del dispositivo. Las medidas de movimiento
proveen información crucial para el reconocimiento de actividades
humanas con un bajo consumo de energía. Los sensores de localización
y podómetro (contador de pasos) también son utilizados pero con ciertas
limitantes tales como la disponibilidad, cobertura y consumo de energía.
Adicionalmente, los teléfonos modernos poseen capacidades de almacenamiento
y procesamiento, además de buena conectividad por lo que son instrumentos
ideales para construir sistemas autónomos.

La capacidad, tamaño diminuto y bajo costo cooperan a bien a los dispositivos
sean portados en cualquier momento y lugar por los usuarios, esto
permitiría recolectar información de manera oportuna y bajo condiciones
realistas, una ventaja bastante ideal a que obtener muestras bajo
supervisión como un experimento de laboratorio \cite{Bao2004}. 

El cuerpo de conocimiento acerca de los sistemas \abbr{HAR} es bastante
extensa, se abarcan varias técnicas de reconocimiento, métodos de
recolección y procesamiento de señales sensoriales \cite{LaraLabrador2012,Kwapisz2011}.
Sin embargo, a pesar de la buena literatura en la arquitectura de
los sistemas \abbr{HAR}, aún es escasa la disponibilidad del software
como un componente abierto que puedan ser utilizados y extendidos
sin depender de librerías (\abbr{API}) privadas, Ej. \emph{Google
Play Services} \cite{Google2016l}, o servicios en Internet (\abbr{SaaS}),
o aplicaciones distribuidas por terceros, Ej. \emph{Sony Lifelog}
\cite{Sony2016l}.

Cualquier iniciativa de software libre hace posible un enfoque colaborativo
varias dimensiones, por un lado la colaboración en construir un componente
de software como una librería, la disponibilidad utilización del mismo,
y también compartir los datos de entrenamiento y evaluación del modelo
de predicción de actividades para una mejora continua.
...
(\{Smartphones}) basados en \abbr{Android} utilizando sensores,
donde se busca aportar un componente de software en forma de librería
de código abierto que sea libremente distribuida. Durante la recolección
de datos y las pruebas se desarrollará adicionalmente una aplicación
móvil que demuestre la efectividad de la librería como componente
de software independiente. También se evaluará el sistema de reconocimiento
de actividades basado en aprendizaje automático en comparación con
...
en la técnica por árboles de decisión (\abbr{DT}). Finalmente, para
...
el diseño e implementación de un sistema \abbr{HAR} utilizando teléfonos
móviles modernos (\{Smartphones}) basados en \abbr{Android}. El
trabajo está organizado en las siguientes partes: en el Capítulo \ref{chap2:marco-teorico}
se definen las ideas generales relevantes al área de estudio para
crear sistemas \abbr{HAR} con miras a generalizar la perspectiva
acerca del problema de abordado. El Capítulo \ref{chap3:Aprendizaje-Automatico}
...
El Capítulo \ref{chap6:evaluaci=0000F3n} presentan los protocolos
realizados para obtener datos de entrenamiento para conducir los experimentos
