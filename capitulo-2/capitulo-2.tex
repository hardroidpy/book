...
cardíaca, conductividad, posicionamiento por \abbr{GPS} y movimientos
...
aprendizaje automático (\abbr{ML}) para construir modelos que puedan
inferir actividades a partir de una gran cantidad de datos medidos
con anterioridad junto con el comportamiento deseado del usuario\cite{Chen2012}.
Esta técnica involucra la creación de modelos de clasificación probabilistas
o estadísticos, seguidos por los procesos de entrenamiento y aprendizaje.
...
realizada durante $w_{j}$ \end{defi}
...
\caption[Proceso HAR]{\label{fig2:harsystem}Flujo general del Reconocimiento de Actividades
Humanas}
 
\end{figure}

En la \figref{fig2:harsystem} se visualiza las fases comunes de las
...
se han propuesto métodos \emph{semi}-supervisados y no supervisados.

Los modelos de Bayes y basados en frecuencia han sido bien cubiertos
en toda la literatura del problema \abbr{HAR}. Estos métodos se resumen
...
\end{tabular}
\par\end{centering}
\caption[Resumen de métodos de aprendizaje agrupados por tipo]{\label{tab2:metodos-aprendizaje}Resumen de métodos de aprendizaje
agrupados por tipo}
\end{table}

Los aspectos relevantes para la selección del algoritmo de aprendizaje
son: el consumo de energía, los requisitos de memoria, la complejidad
...
un alto rendimiento\cite{ReyesOrtiz2015}.

En el capitulo \ref{chap3:Aprendizaje-Automatico} se detalla lo referente
a los métodos de aprendizaje disponibles y el método utilizado en
este trabajo.
