...
elementos requeridos para implementar los sistemas \abbr{HAR}. Desde
...
de ingesta de datos. Este nuevo medio abre nuevas posibilidades de
...
Los sensores de atuendo (\emph{véase \abbr{Wearables}}) obtienen
...
\abbr{Bluetooth}) para la transmisión de señales. Las señales obtenidas
...
\item [{Ubicación}] miden datos obtenidos con las redes celulares \abbr{3G}
y los satélites de navegación \abbr{GPS}. Provee información de contexto
...
(\abbr{HRM}), la temperatura del cuerpo, el ritmo de respiración,
entre otros.
...
envío de mensajes de texto (\abbr{SMS}) y multimedia (\abbr{MMS}).
...
incorporado más capacidad de computación (\abbr{CPU}), más \emph{hardware}
...
de imágenes y vídeos. En la \tabref{tab2:capacidad-movil} se listan
...
Es decir, utilizar algoritmos de aprendizaje automático (\abbr{ML})
...
descritos son un fundamento para comprender el problema de \abbr{HAR}
