...
serán obtenidos a futuro. Rememorando el \ref{chap:Aprendizaje-Automatico},
...
clasificador (\emph{classifier}) \cite{Rajaraman2011}. En esta sección
se detalla más sobre cómo los clasificadores supervisados son construidos
a partir del conjunto de entrenamiento que incluye información acerca
de la manera correcta de clasificar un subconjunto de datos.
...
de entrenamiento (\emph{training set}) disponible y separar un remanente
conocido como conjunto de validación (\emph{test set}). El problema
de los algoritmos de \abbr{ML} es el sobre ajuste (\emph{overfit}),
