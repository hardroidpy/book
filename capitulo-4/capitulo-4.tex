...
\label{def4:moving-average}\newtheorem{defi}{Definición}\begin{defi}(\emph{Moving
average}) Dada una secuencia $\left\{ a_{i}\right\} _{i=1}^{N}$,
...
\hline 
$w$ & $\mathrm{f}_{0}$ & $\ldots$ & $\mathrm{f}_{m}$\tabularnewline
\hline 
\hline 
...
aplicación de aprendizaje automático \abbr{ML} donde depende de un
...
observaciones denominadas instancias \cite{LaraLabrador2013}. El
propósito principal del algoritmo es de clasificar los datos que serán
obtenidos a futuro. La entrada aplicada a un algoritmo de \abbr{ML}
es un conjunto de entrenamiento (\emph{training set}) y el resultado
del mismo es un clasificador (\emph{classifier}) \cite{Rajaraman2011}.
...
La tarea de etiquetado debe ser realizado de forma manual como en
la sección \ref{ssec44:labeling} o si es posible de forma automática.
...
automático supervisados \abbr{ML} \cite{Rajaraman2011}.

\label{def4:clasificacion}\newtheorem{defi}{Definición}\begin{defi}(Clasificador
\abbr{ML}) Un algoritmo \abbr{ML} recibe un conjunto entrenamiento
compuesto por un conjunto de pares $(x,y)$ conocidas como muestras
...
En el capítulo \ref{chap3:Aprendizaje-Automatico} se construye la
función $f$ por medio de un algoritmo con un enfoque basado en arboles
de decisión (\abbr{DT}). La forma de f es un árbol, y cada nodo del
árbol tiene una función de x que determina que sobre qué nodo hijo
la búsqueda procede, cada nodo puede tener cualquier número de nodos
hijos. Los árboles de decisión son preferibles para las clasificación
binaria o de multi-clases, especialmente cuando la dimensión del vector
característicos no es muy grande \cite{Rajaraman2011}.

\subsubsection{Reconocimiento}

Explicar la utilización de la funcion F para clasificar y poder retroalimentar
