...
\begin{figure}[!tbph]
\centering{}\includegraphics[width=1\linewidth]{capitulo-4/graphics/diagrama_4_1}\caption[Arquitectura de sistema \abbr{HAR}]{\label{fig4:componentes-har}Componentes de los sistemas \abbr{HAR}}
\end{figure}
...
métodos discretos de\emph{Fourier} utilizando algoritmos \abbr{FFT}
con números reales. 
...
a futuro. La entrada aplicada a un algoritmo de \abbr{ML} es un conjunto
...
así como se muestra en la tabla \tabref{tab4:labeled}. La tarea de
...
\label{def4:clasificacion}\newtheorem{defi}{Definición}\begin{defi}(Clasificador\abbr{ML})
Un algoritmo\abbr{ML} recibe un conjunto entrenamiento compuesto
...
de decisión (\abbr{DT}). La forma de $f$ es un árbol, y cada nodo
