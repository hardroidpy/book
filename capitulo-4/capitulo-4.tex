...
utilizados como instrumentos\footnote{\emph{hardware}}.
...
se realiza según la necesidad (\emph{Off-line learning}).
...
\begin{figure}[!tbph]
\centering{}\includegraphics[width=1\linewidth]{capitulo-4/graphics/diagrama_4_1}\caption[Componentes \abbr{HAR}]{\label{fig4:componentes-har}Componentes de los sistemas \abbr{HAR}}
\end{figure}
...
posición, ambiente y fisiológicas. A continuación se describen en
detalle cada grupo.
...
a un algoritmo de \abbr{ML} es un conjunto de entrenamiento (\emph{training
set}) y el resultado del mismo es un clasificador (\emph{classifier})
\cite{Rajaraman2011}. En esta sección se detallan los pasos comunes
de construcción de un clasificador supervisado los cuales se listan
a continuación :
\begin{itemize}
\item Preparación
\item Aprendizaje
...
El proceso de aprendizaje puede realizarse en base a tres métodos
\cite{Rajaraman2011}.
...
completo para construir un modelo una sola vez. Por otro lado el aprendizaje
\emph{en linea} puede contar con los datos de entrenamiento a medida
que se disponen los mismos, como en un flujo de datos son examinados
una sola vez y que no pueden ser re-visitados. Con este método se
mantiene el modelo constantemente, a medida que nuevas instancias
de entrenamiento son recibidas, se tiene la elección de modificar
el modelo para mejorarlo con nuevas instancias. 

Las ventajas de realizar el aprendizaje en linea se pueden apreciar
a simple vista. Se puede manejar un conjunto de entrenamiento de gran
tamaño sin necesidad de acceder a mas de una instancia al mismo tiempo.
También se puede adaptar a los cambios en la población de las instancias
de entrenamiento a medida que transcurre el tiempo.

Una mejora al aprendizaje en linea es el aprendizaje \emph{activo}.
El clasificador se construye utilizando instancias de entrenamiento,
pero también puede recibir datos no clasificados para que el modelo
los clasifique. Si el clasificador realiza una clasificación con mucha
incertidumbre, el clasificador puede requerir que la instancia sea
verificada para aseverar su clasificación verdadera a un costo significativo. 

Por ejemplo, la instancia puede ser retenida y verificada manualmente
por medio de una encuesta a personas reales. De esta manera las instancias
verificadas pueden ser luego utilizadas para mejorar el clasificador
(ver la \figref{fig4:componentes-har}). En este trabajo se busca
tener nuevas instancias de entrenamiento por medio de un proceso colaborativo,
partiendo inicialmente de un conjunto de instancias de entrenamiento
e ir mejorando la población con una encuesta dirigida por medio del
uso del clasificador de \abbr{HAR}.
...
de entrada disponibles en un conjunto de entrenamiento (\emph{training
set}) y un remanente conocido como conjunto de validación (\emph{test
set}). El problema común de los algoritmos de \abbr{ML} es el error
asociado al sobre ajuste (\emph{overfit}), es decir, la clasificación
resulta como se espera en el conjunto de entrenamiento pero ante la
mínima variación con datos desconocidos de una población mayor el
resultado se torna atípico o con errores de clasificación \cite{Rajaraman2011}. 

El conjunto de validación permite evaluar el clasificador determinando
si se sobre ajustan los resultados y cuanta precisión posee el modelo.
La precisión consiste es la proporción de instancias correctamente
clasificadas. El sobre ajuste es una medida del error en base a la
precisión del algoritmo con instancias en el conjunto de evaluación.
