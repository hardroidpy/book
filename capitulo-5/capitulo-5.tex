...
sistema. La interfaz para programadores (\emph{Application Programming
...
\abbr{HAR}. El servicio de la plataforma \abbr{Android} es una aplicación
de segundo plano para teléfonos móviles que implementa algoritmos
...
para teléfonos móviles \abbr{Android}\emph{ }se dispone de capacidades
...
\par\end{centering}
\caption[Modelo de dos capas]{\label{fig5:cliente_servidor}Modelo de dos capas (CAMBIAR COLORES
COMO FIGURA 5.2)}
\end{figure}
...
\par\end{centering}
\caption[Modelo de tres capas]{\label{fig5:tres_capas}Modelo de tres capas (PONER FLECHAS DE INTERACCION
COMO 5.1)}
...
\abbr{DVM}) de propósito específico diseñada especialmente para \abbr{Android}
de manera que este corra aplicaciones programadas en el lenguaje \abbr{Java}
...
al de las bases de datos \abbr{SQL}. Los datos pueden ser persistidos
...
es realizado por medio de un mecanismo denominado \emph{Inter-Process
...
una interfaz \emph{Android Interface Definition Language} (\abbr{AIDL})
parte del mecanismo \emph{IPC Binder} (ver sección \ref{ssec52:framework}).
...
\caption{\label{alg5:reconocimiento}Detección de actividades humanas (VER
NUMEROS DE LINEA Y TAMAÑO DEL ALGORITMO)}
\end{algorithm}
...
descargada de Internet. El mecanismo detallado se describe en el {[}REFERENCIA
A GranNRun{]}.
...
del servicio \emph{HARDroid} desarrollado y evaluar los resultados
...
permiten validar las funcionalidades de \emph{HARDroid} y además encuestar
...
conexión persistente con el servicio \emph{HARDroid}. El servicio
...
plataforma \abbr{Android} y su interacción \cite{Gargenta2014}. 
...
proveído por el marco de trabajo de \abbr{Android}. Los desarrollos
