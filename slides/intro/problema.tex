...
\onslide<1> 
\begin{quote}
Determinar las acciones o comportamientos de uno o más individuos
...
inglés \emph{(Human Activity Recognition})
\end{block}
\end{overprint}
\begin{center}
...
\begin{itemize}[<+->]
\item Es un tópico de investigación multidisciplinario que busca diseñar
algoritmos para:
\begin{itemize}
\item Capturar movimientos de uno o más individuos en interacción con su
entorno 
\item Realizar aprendizaje e inferencia
\item Proveer información de contexto 
\end{itemize}
\end{itemize}
\begin{overprint}
\onslide<2> 
...
\framesubtitle{Motivación\setlength\columnsep{0pt}}
\begin{columns}

\column{0.5\textwidth}
\begin{itemize}[<+->]
\item El \structure{contexto} es primordial para los sistemas inteligentes 
\item Avances en tecnologías de \structure{computación móvil} y \structure{sensores} 
\item Uso intensivo de \structure{teléfonos móviles modernos} en la vida
diaria.
\item \setbeamercovered{transparent}Popularidad de \structure{aplicaciones móviles}
de contexto en 
\begin{itemize}
...
\end{frame}
%
\begin{frame}[shrink]{Definición del Problema}

\framesubtitle{Actividades Humanas}

\setbeamercovered{transparent}
\begin{block}{Actividades Simples}
Acciones simples periódicas de larga duración.
\end{block}
\begin{center}
...
Acciones cortas o periódicas no comunes.
\end{block}
\begin{center}
...
\hline 
Gimnasio & Alzar pesas, ejercicio aeróbico, bicicleta estática.\tabularnewline
\hline 
...
\begin{frame}{Definición del Problema}
\framesubtitle{Definición Matemática}

	\begin{definition}[Problema HAR relajado]
	Dado (1) un conjunto $W=\{w_{0},...,w_{m-1}\}$ de $m$ ventanas de tiempo
